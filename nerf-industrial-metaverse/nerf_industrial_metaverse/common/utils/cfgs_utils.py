...
def remove_yaml_key(file_path, key_to_remove):
    with open(file_path, 'r') as file:
        config = yaml.safe_load(file)

    keys = key_to_remove.split('.')
    current = config
    for k in keys[:-1]:
...
def update_yaml_config(file_path, new_config):
    with open(file_path, 'r') as file:
        config = yaml.safe_load(file)

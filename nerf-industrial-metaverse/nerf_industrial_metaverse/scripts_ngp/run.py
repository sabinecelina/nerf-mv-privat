...
def entrypoint():
        ...
        with tqdm(desc="Training", total=n_steps, unit="steps") as t:
            while testbed.frame():
                ...
                if now - tqdm_last_update > 0.1:
                    ...
                    # logger.add_log(f'progress step: {testbed.training_step}, loss: {testbed.loss}', level="TRAINING")
    else:
    ...
    if valid_key_in_cfgs(args, 'video_camera_path'):
        ...
        for i in tqdm(list(range(min(n_frames, n_frames + 1))), unit="frames", desc=f"Rendering video"):
            ...
            # logger.add_log(f'video_progress current_frame: {i} total_frames: {n_frames}', level="RENDERING")
